...
def RR():
    ...
    #r=np.loadtxt('/home/bellis/Downloads/adr10dat_small.txt')
    r=np.loadtxt('/home/bellis/Downloads/adr10dat.txt')
    ra=r[:,0]
    dec=r[:,1]
    z=r[:,2]
    ...
    paras = np.zeros(chunk_size*ngals_for_calculation/2)
    perps = np.zeros(chunk_size*ngals_for_calculation/2)
    ...
    for j in xrange(nchunks):
        ...
        for i in range(lo,hi):
            if i!=ngals-1:
                ...
                #print len(paras)
                #print len(R_para1)
                paras[k:k+ncalcs] = R_para1
                perps[k:k+ncalcs] = R_perp1
